...
class ActionHandlerMixin:
    ...
    async def _get_content_body_for_action(self, action: Awaitable, content: dict) -> dict:
        body: dict[str, Any] = {}
    ...
    async def _get_required_params(method: Awaitable) -> set[str]:
        ...
        params = {key for key in annotations.keys() if 'Optional' not in str(annotations[key])}
    ...
    async def _get_optional_params(method: Awaitable) -> set[str]:
        annotations: dict[str, Any] = method.__annotations__
        params = {key for key in annotations.keys() if 'Optional' in str(annotations[key])}
        params.discard('return')
        return params
